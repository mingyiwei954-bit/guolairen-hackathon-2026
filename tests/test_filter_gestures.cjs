...
let f=fixture();for(let i=0;i<15;i++){f.wheel(-1);f.scroll(-1);}f.settle();assert.equal(f.ctx.filterVisibilityProgress,0,'one gesture with many events counts once');
f.wheel(-.001,300);f.settle();assert.equal(f.ctx.filterVisibilityProgress,1,'second tiny return counts with no distance threshold');
f.wheel(.001,300);f.settle();assert.equal(f.ctx.filterVisibilityProgress,0,'downward reversal resets');
f.wheel(-.001,300);f.settle();assert.equal(f.ctx.filterVisibilityProgress,0);f.wheel(-.001,300);f.settle();assert.equal(f.ctx.filterVisibilityProgress,1);
f=fixture();f.touchStart();f.touchMove(200.1);f.touchMove(200.2);f.touchEnd();f.settle();assert.equal(f.ctx.filterVisibilityProgress,0);
f.touchStart();f.touchMove(200.01);f.touchEnd();f.settle();assert.equal(f.ctx.filterVisibilityProgress,1,'separate tiny touch strokes reveal');
f=fixture();f.wheel(-20,300);f.wheel(2);f.wheel(-20);f.settle();assert.equal(f.ctx.filterVisibilityProgress,0,'up/down/up within one stroke is not two consecutive up strokes');
f=fixture();const key=(repeat=false)=>f.handlers.keydown({key:'ArrowUp',repeat,target:{closest:()=>null}});key();key(true);f.settle();assert.equal(f.ctx.filterVisibilityProgress,0);key();f.settle();assert.equal(f.ctx.filterVisibilityProgress,1);
// Time-based appearance keeps going without more scroll distance or release.
f=fixture(0,500,false);f.wheel(-.1,300);f.wheel(-.1,300);f.frame(120);assert(f.ctx.filterVisibilityProgress>0&&f.ctx.filterVisibilityProgress<1);f.wheel(-.1,20);f.frame(260);assert.equal(f.ctx.filterVisibilityProgress,1,'same-direction input cannot keep restarting appearance');
...
f=fixture(1);for(let cycle=0;cycle<30;cycle++){
 ...
 f.wheel(-.01,300);f.settle();assert.equal(f.ctx.filterVisibilityProgress,0);f.wheel(-.01,300);f.settle();assert.equal(f.ctx.filterVisibilityProgress,1);assert.equal(f.viewport.scrollTop,before);
}
...
f=fixture(0,500,false);f.wheel(-20,300);f.wheel(-20,300);f.ctx.cleanupFilterControls();f.settle();assert.equal(f.ctx.filterVisibilityProgress,0);assert.equal(Object.keys(f.handlers).length,0);
...
console.log('Filter flow: 30 stable cycles, no distance threshold, time-based fade, top priority, overscroll, no layout drift, channel reveal and cleanup passed.');
